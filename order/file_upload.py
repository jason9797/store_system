...
def customer_file(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            customerfile = CustomerFile(file=request.FILES['file'],
            ...
            if file_type in ['xls', 'xlsx']:
                ...
                for i in range(nrows):
                    ...
                    contact.save()
            return HttpResponse(
                "<script language='javascript'>alert('上传成功');history.go(-1)</script>")
        else:
            return HttpResponse(
                "<script language='javascript'>alert('未知错误');history.go(-1)</script>")
    else:
...
def order_file(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #     #print file,filename
            #     name=request.FILES[filename].name
            orderfile = OrderFile(file=request.FILES['file'],
                                  title=request.POST.get('title'))
            orderfile.save()
            ...
            if file_type in ['xls', 'xlsx']:
                ...
                for i in range(1, nrows):
                    ...
                    try:
                        if isinstance(user, int) or isinstance(user, float):
                            user = str(int(user))
                    ...
                    order = Order(delivery_no=order_no,
                                  customer=customer,
