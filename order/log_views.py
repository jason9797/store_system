...
def user_log_info(request):
    if request.method=="POST":
        ...
        initial={}
        if username:
            user_log=UserLog.objects.filter(user=User.objects.get(pk=username))
        ...
        if page:
            ...
            return render(request,'user_log_pagination.html',{'user_log':user_log
                                              })
    ...
    return render(request,'user_log_info.html',{'form':log_form,'filter':filter_dict,
                                                'user_log':user_log,'total_page':total_page,'current_page':page})
@admin_required(login_url='/login')
def get_object_info(request):
    ...
    return render(request,'get_object_info.html',{'data':data,'object':user_log})
