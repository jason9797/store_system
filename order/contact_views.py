...
def get_contact_address(request):
    ...
    if request.user.is_superuser:
        ...
        contact_address = Contact_info.objects.filter(
...
def get_contact_phone(request):
    ...
    if request.user.is_superuser:
        ...
        contact_phone = Contact_info.objects.filter(
            customer=Customer.objects.filter(
                Q(user=request.user) | Q(user__isnull=True))).values(
                    'phone_number')
...
def alert_message(request):
    ...
    return HttpResponse(
        "<script language='javascript'>alert('提醒成功');window.location.href=document.referrer;</script>")
...
def mark_all_message(request):
    request.user.notifications.mark_all_as_read()
    return HttpResponse(
        "<script language='javascript'>alert('标记成功');window.location.href=document.referrer;</script>")