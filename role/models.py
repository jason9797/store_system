...
class User_goal(models.Model):
    ...
    goal_quantity = models.IntegerField(verbose_name="目标量",
                                        null=True,
                                        blank=True)
    goal_money = models.DecimalField(verbose_name="目标成交额",
                                     max_digits=7,
                                     decimal_places=2,
                                     default=0,
                                     null=True,
...
class Issuing_person_cut(models.Model):
    ...
    cut_percentage = models.DecimalField(verbose_name="提成系数",
                                         max_digits=2,
                                         decimal_places=2,
                                         default=0,
                                         null=True,
...
def get_this_today(self):
    ...
    if self.is_superuser:
        today_order = Order.objects.filter(
            Q(customer__in=Customer.objects.filter(user__is_superuser=False)) &
            Q(jointime__gte=today))
    else:
        if self.id in [i[0] for i in Customer.objects.filter(user__isnull=False).values_list("user").distinct()]:
            today_order = Order.objects.filter(
                Q(customer__in=Customer.objects.filter(user=self)) &
                Q(jointime__gte=today))
        else:
            return (0, 0)
...
def get_this_week(self):
    week_monday = datetime.date.today() - datetime.timedelta(
        days=datetime.date.today().weekday())
    if self.is_superuser:
        this_week_order = Order.objects.filter(
            Q(customer__in=Customer.objects.filter(user__is_superuser=False)) &
            Q(jointime__gte=week_monday))
    else:
        if self.id in [i[0] for i in Customer.objects.filter(user__isnull=False).values_list("user").distinct()]:
            this_week_order = Order.objects.filter(
                Q(customer__in=Customer.objects.filter(user=self)) &
...
def get_this_month(self):
    this_month_start = datetime.date(datetime.date.today().year,
                                     datetime.date.today().month, 1)
    if self.is_superuser:
        this_month_order = Order.objects.filter(
            Q(customer__in=Customer.objects.filter(user__is_superuser=False)) &
            Q(jointime__gte=this_month_start))
    else:
        if self.id in [i[0] for i in Customer.objects.filter(user__isnull=False).values_list("user").distinct()]:
            this_month_order = Order.objects.filter(
                Q(customer__in=Customer.objects.filter(user=self)) &
...
def get_order_success(self):
    ...
    if self.is_superuser:
        this_month_order = Order.objects.filter(
            Q(customer__in=Customer.objects.filter(user__is_superuser=False)) &
            Q(jointime__gte=this_month_start)).count()
        this_month_success = Order.objects.filter(Q(customer__in=Customer.objects.filter(
            user__is_superuser=False)) & Q(jointime__gte=this_month_start) & Q(
            state=Order_State.objects.get(name=_('已签收')))).count()
    else:
        this_month_order = Order.objects.filter(
            Q(customer__in=Customer.objects.filter(user=self)) &
            Q(jointime__gte=this_month_start)).count()
    ...
    try:
        success_per = "%.2f%%" % (
            (float(this_month_success) / float(this_month_order)) * 100)
    except:
        success_per = 0
    return success_per
...
def get_team_goal(self):
    ...
    goal_money = User_goal.objects.filter(user__in=team_user).aggregate(
        Sum('goal_money'))['goal_money__sum']
    return goal_money
...
def get_team_order(self):
    this_month_start = datetime.date(datetime.date.today().year,
                                     datetime.date.today().month, 1)
    team_user = self.groups.all()[0].user_set.all()
    team_month_order = Order.objects.filter(
        Q(customer__in=Customer.objects.filter(user__in=team_user)) &
        Q(jointime__gte=this_month_start))
    ...
    for i in team_user:
        this_day_order = Order.objects.filter(Q(
            customer__in=Customer.objects.filter(user=i)) & Q(
                jointime__gte=today))
        if this_day_order:
            this_day_order = this_day_order.aggregate(
                sum_order=Sum("product__price"))
...
def this_week_rank(self):
    ...
    for i in team_user:
        ...
        if this_week_order:
            this_week_order = this_week_order.aggregate(
                sum_order=Sum("product__price"))
...
def this_month_rank(self):
    month_start = datetime.date(datetime.date.today().year,
                                datetime.date.today().month, 1)
    team_user = self.groups.all()[0].user_set.all()
    rank_list = []
    for i in team_user:
        ...
        if this_month_order:
            this_month_order = this_month_order.aggregate(
                sum_order=Sum("product__price"))
...
def person_quantity_rank(self):
    month_start = datetime.date(datetime.date.today().year,
                                datetime.date.today().month, 1)
    this_month_order = Order.objects.filter(Q(jointime__gte=month_start))
    quantity_person = this_month_order.values("customer__user").annotate(
        quantity=Count("customer__user")).order_by("-quantity")
...
def person_money_rank(self):
    ...
    money_person = this_month_order.values("customer__user").annotate(
        money=Sum("product__price")).order_by("-money")
...
def get_team_rank(self):
    ...
    for i in groups:
        this_month_order = Order.objects.filter(
            Q(customer__in=Customer.objects.filter(user__in=i.user_set.all())) &
            Q(jointime__gte=month_start))
        if this_month_order:
            total_money = this_month_order.aggregate(
                money=Sum("product__price"))['money']
...
def get_failed_order(self):
    month_start = datetime.date(datetime.date.today().year,
                                datetime.date.today().month, 1)
    this_month_order = Order.objects.filter(
        Q(state=Order_State.objects.filter(~Q(name="已签收") & Q(level__gte=3))) &
        Q(jointime__gte=month_start)).order_by("-jointime")
...
def get_no_approval_order(self):
    month_start = datetime.date(datetime.date.today().year,
                                datetime.date.today().month, 1)
    this_month_order = Order.objects.filter(
        Q(state=Order_State.objects.filter(level=1)) &
        Q(jointime__gte=month_start)).order_by("-jointime")
    return this_month_order
...
def get_this_week_success(self):
    ...
    try:
        success_per = "%.2f%%" % (
            (float(this_week_success) / float(this_week_order)) * 100)
    except:
        success_per = 0
    return success_per
...
def get_last_week_success(self):
    last_week_monday = datetime.date.today() - datetime.timedelta(
        days=(datetime.date.today().weekday() + 7))
    last_week_order = Order.objects.filter(
        Q(jointime__gte=last_week_monday)).count()
    last_week_success = Order.objects.filter(Q(jointime__gte=last_week_monday) & Q(state=Order_State.objects.get(name=_('已签收')))).count()
    try:
        success_per = "%.2f%%" % (
            (float(last_week_success) / float(last_week_order)) * 100)
    except:
        success_per = 0
    return success_per
...
def get_last_month_success(self):
    last_month_start = datetime.date(datetime.date.today().year,
                                     datetime.date.today().month, 1)
    last_month_order = Order.objects.filter(
        Q(jointime__gte=last_month_start)).count()
    ...
    try:
        success_per = "%.2f%%" % (
            (float(last_month_success) / float(last_month_order)) * 100)
    except:
        success_per = 0
    return success_per
