...
def stock_add(request):
    ...
    form_list = {'stock_form': stock_form,}
...
def stock_remove(request):
    ...
    response.write(
        "<script language='javascript'>alert('删除成功!');window.location.href='/stock/other';</script>")
    return response
    ...
    if request.method == "POST":
        ...
        if request.GET.get('id'):
            stock_type_form = Stock_TypeForm(
                model_to_dict(Stock_Type.objects.get(pk=request.GET.get('id'))))
        else:
            stock_type_form = Stock_TypeForm()
...
def stock_type_remove(request):
    ...
    response = HttpResponse()
    response.write(
        "<script language='javascript'>alert('删除成功!');window.location.href='/stock/type';</script>")
    return response
...
def stock_channel(request):
    if request.method == "POST":
        ...
        if stock_channel_form.is_valid():
            ...
            if request.GET.get("id"):
                stock_channel = Stock_Channel.objects.get(
                ...
                stock_channel = Stock_Channel(company=company,
                                              person=person,
        ...
        if request.GET.get('id'):
            stock_channel_form = Stock_ChannelForm(model_to_dict(
                Stock_Channel.objects.get(pk=request.GET.get('id'))))
        else:
...
def stock_channel_remove(request):
    ...
    response = HttpResponse()
    response.write(
        "<script language='javascript'>alert('删除成功!');window.location.href='/stock/channel';</script>")
    return response
...
def stock_management(request):
    if request.method == 'POST':
        ...
        initial = {}
        if stock_mode:
            stock_management = Stock_Management.objects.filter(
                stock_mode=eval(stock_mode))
        ...
        if mode:
            stock_management = stock_management.filter(
                mode=Stock_Mode.objects.get(pk=int(mode)))
        ...
        if page:
            ...
            return render(request, 'stock_management_pagination.html', {
    ...
    if request.method == "POST":
        ...
        if request.GET.get('id'):
            stock_mode_form = Stock_ModeForm(
                model_to_dict(Stock_Mode.objects.get(pk=request.GET.get('id'))))
        else:
            stock_mode_form = Stock_ModeForm()
...
def stock_order_no(request):
    if request.method == "POST":
        ...
        if order_no:
            ...
            return HttpResponse(
                "<script language='javascript'>alert('订单号不能为空!');window.history.go(-1);</script>")
    else:
        order_no_null = Order.objects.filter(delivery_no='').order_by(
            '-jointime')
        order_no_success = Order.objects.exclude(
            delivery_no__isnull=True).exclude(delivery_no__exact='')
        form_list = {
...
def stock_info(request):
    if request.method == 'POST':
        ...
        if page:
            ...
            return render(request, 'stock_pagination.html', {
                'stock': stock,
                'stock_type': stock_type,
                'stock_channel': stock_channel
            })
